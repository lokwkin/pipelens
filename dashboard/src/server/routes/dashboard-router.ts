...
export function setupDashboardRoutes(storageAdapter: StorageAdapter, upload: multer.Multer): Router {
  ...
  // Import from JSON feature (used by React frontend)
  router.post('/import', upload.array('files', 10), async (req: Request, res: Response) => {
    try {
      ...
      const results: Array<{ fileName: string; status: string; message: string; data?: any }> = [];
      ...
      // Return response matching the React frontend expectations
      res.json({
        success: true,
        results,
      });
