...
class TestStepAsync:
    ...
    async def test_constructor_with_parent(self):
        ...
        async def parent_func(st):
            ...
            inner_child_step = await st.step(
                "child", lambda child: asyncio.sleep(0.001, result=child)
            )
    ...
    async def test_run_return_result(self):
        ...
        async def test_func(st):
            inner_result = await st.step(
                "inner-step", lambda _: asyncio.sleep(0.001, result="result")
            )
    ...
    async def test_track_time_usage(self):
        ...
        async def test_func(st):
            await st.step(
                "inner-step", lambda _st: asyncio.sleep(0.02, result="result")
            )
            return st
    ...
    async def test_event_step_start(self):
        ...
        async def test_func(st):
            ...
            await st.step(
                "inner-step", lambda inner_st: asyncio.sleep(0.001, result=inner_st)
            )
            return st
    ...
    async def test_event_step_success(self):
        ...
        async def test_func(st):
            inner_result = await st.step(
                "inner-step",
                lambda inner_st: asyncio.sleep(0.001, result="inner-result"),
            )
